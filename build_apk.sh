...
java -cp "$ANDROID_TOOLS_JAR:build/tools" SignApk build/unsigned.apk build/BonsaiLocal-1.2.1-arm64.apk "$KEYSTORE" "$ALIAS" "$STOREPASS" "$KEYPASS" "$PREV"
TEST_APK=build/BonsaiLocal-1.2.1-arm64.apk python3 -m pytest -q
