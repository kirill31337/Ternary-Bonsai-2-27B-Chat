...
put('com/prismml/bonsailocal/repair/Transfers','''public class Transfers {public static final String PAUSE="pause",DOWNLOAD="download",IMPORT="import";public static volatile boolean busy,paused,release,slowStop;public static volatile int performed,failed;public static void reset(){busy=paused=release=slowStop=false;performed=failed=0;}public static void configure(android.content.Context c){}public static void fail(Throwable e){failed++;}public static String describe(){return "test";}public static int percent(){return 0;}public static void pause(){paused=true;}public static boolean begin(android.content.Context c){if(busy)return false;busy=true;return true;}public static void end(){busy=false;}public static void perform(String a,android.net.Uri u){performed++;while(!release&&(slowStop||!paused))try{Thread.sleep(5);}catch(InterruptedException e){break;}}public static void release(android.net.Uri u){}}''')
put('com/prismml/bonsailocal/repair/ServiceHostTest','''public class ServiceHostTest {
 ...
 public static void main(String[]x)throws Exception {
  ...
  s=make();Transfers.slowStop=true;send(s,Transfers.DOWNLOAD,7);await(()->Transfers.performed==1);s.onTimeout(7,1);
  ok(s.stopped&&!((android.app.Service)s).foreground&&android.os.PowerManager.held==0,"timeout must stop foreground service immediately even when I/O is blocked");
  ok(Transfers.paused&&Transfers.busy,"timeout must pause and retain the worker reservation until I/O exits");
  s.onDestroy();Transfers.release=true;await(()->!Transfers.busy);System.out.println("PASS Android 15 timeout stops service before blocked I/O completes");
  s=make();s.onTimeout(8,1);ok(s.stopped&&!((android.app.Service)s).foreground&&!Transfers.busy,"timeout with no worker must clean up");System.out.println("PASS timeout before work cleans up");
  System.out.println("6 host service lifecycle tests passed (Android boundary doubles, not device test)");
 }
}''')
...
subprocess.run(['java','-cp',str(classes),'com.prismml.bonsailocal.repair.ServiceHostTest'],check=True,timeout=15)
