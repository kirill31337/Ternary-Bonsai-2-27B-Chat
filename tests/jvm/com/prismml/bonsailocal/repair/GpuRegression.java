...
public final class GpuRegression {
 ...
 public static void main(String[] args)throws Exception{
  ...
  try{
   ...
   String logger=System.getenv("BONSAI_TEST_PRISM_LOGGER");
   if(logger==null)throw new AssertionError("Run tests/build_prism_log_fixture.sh and set BONSAI_TEST_PRISM_LOGGER");
   String loggedScript="#!/bin/sh\nprintf '%s\\n' \"$@\" > '"+argv+"'\nprintf '%s' \"$GGML_BACKEND_PATH\" > '"+env+"'\n'"+logger+"' \"$@\"\ncase \" $* \" in *' --list-devices '*) exit 0;; esac\nexec sleep 60\n";
   Files.writeString(exe,loggedScript);
   ...
   check(v.get(v.indexOf("--log-verbosity")+1).equals("4"),"GPU startup requests the Prism level needed for buffer evidence");
   b.stop();await(b,5);
   String startupLog=Files.readString(files.resolve("server.log"));
   b.selfTest();
   for(int i=0;i<100&&!b.status().contains("\"exit\":0,");i++)Thread.sleep(100);
   check(b.status().contains("\"exit\":0,"),"GPU device probe completes successfully");
   check(Files.readString(files.resolve("server.log")).equals(startupLog),"engine self-test preserves the model startup log");
   check(Files.readString(files.resolve("engine-test.log")).contains("Vulkan0"),"device probe has its own diagnostic log");
